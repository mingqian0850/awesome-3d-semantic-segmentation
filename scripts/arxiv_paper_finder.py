#!/usr/bin/env python3
"""arXiv paper watcher for 3D semantic segmentation.

Queries the arXiv API for recent papers on 3D semantic segmentation
(point clouds / LiDAR / voxels / range images), excludes papers already
listed in the repo, and emits a markdown candidate list.

Intended to run inside a GitHub Actions workflow (paper-watch.yml), but can
also be run locally:

    python3 scripts/arxiv_paper_finder.py --days 7 --output new_papers.md
...
# Search: CS category, "semantic segmentation" in abstract, plus a 3D signal.
# `abs:` fields are searched as phrases; `cat:` restricts the category.
QUERY = (
    'cat:cs.CV AND '
...
def fetch_arxiv(max_results: int, sort_by: str = "submittedDate", retries: int = 3, api_url: str = ARXIV_API) -> ET.Element:
    ...
    for attempt in range(1, retries + 1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "awesome-3d-semantic-segmentation-watcher/1.0"})
...
def already_listed(arxiv_id: str, known_ids: set) -> bool:
    return arxiv_id in known_ids
...
def parse_entry(entry: ET.Element) -> dict:
    ...
    for l in entry.findall("atom:link", NS):
        if l.get("title") == "pdf" or (l.get("rel") == "alternate" and "abs" in (l.get("href") or "")):
            link = l.get("href", "")
...
def main() -> int:
    ...
    parser.add_argument("--max", type=int, default=60, help="max results to fetch from arXiv")
    ...
    if not candidates:
        ...
        print(f"No new papers in the last {args.days} days (checked {len(entries)} fetched entries).")
        return 0

    lines = [
        f"# 📚 New 3D semantic segmentation papers (auto-detected)",
        "",
        f"Fetched {len(entries)} arXiv entries; **{len(candidates)} new candidate(s)** in the last {args.days} days.",
        "",
